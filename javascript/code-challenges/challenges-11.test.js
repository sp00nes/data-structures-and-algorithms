...
function transformToLis(obj) {
  ...
  for (let [key, value] of Object.entries(obj)) {
    returnArr.push(`<li>${key}: ${value}</li>`);
  }
  return returnArr;
}
...
const totalSum = (input) => {
  let output = 0;
  input.map((elem) => { output = elem.reduce((acc, val) => acc + val) + output; });
  return output;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let output = [];
  output = input.map(arr => arr.filter(num => typeof num !== 'string').filter(num => num % 5 === 0).map(num => Math.pow(2, num)));
  return output;
};
...
let findMaleAndFemale = (data) => {
  let output = '';
  output = data.map(obj => {if (obj.gender === 'female' || obj.gender ==='male'){return obj.name;}else{return [];}}).filter(x => typeof x === 'string').join(' and ');
  return output;
};
